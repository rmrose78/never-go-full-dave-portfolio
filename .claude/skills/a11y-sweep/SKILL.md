...
- **Color contrast is effectively unchecked by jest-axe.** axe-core's own
  ...
  backgrounds that a hand-maintained hex-math utility can't compute (it
  only knows flat color pairs you hand-enter).
...
via Playwright's `webServer` config, if this project has one set up.)

## Efficiency note

This sweep is tool-call-dense (real-browser navigation, axe-core scans
across multiple routes/viewports, optional Lighthouse runs). Delegate the
whole sweep to a background Agent call rather than running it inline in
the main conversation — have the subagent execute the steps below and
return only the condensed report format at the bottom, keeping the raw
violation dumps and intermediate tool output out of the main thread's
context.
...
1. **Run the automated suite**: `npm run test:a11y`. This is
   ...
   desktop (1440px) viewports, plus any interactive-open states worth
   covering (mobile nav menu open, a modal open, etc.) — the same "one
   check per meaningfully distinct render state" rule this repo already
   applies to jest-axe.
...
3. **If violations show up in only one test but not others** (e.g. only a
   modal-open case), suspect a test-timing issue before a real design bug —
   compute the actual CSS-declared contrast for the colors involved and
   compare against what axe reported. A meaningful mismatch usually means
   axe sampled mid-animation or mid-transition; wait for the relevant
   transition/animation to finish (`el.getAnimations().map(a => a.finished)`)
   and re-run before concluding it's a real violation.
4. **Optionally run Lighthouse**: `npm run lighthouse:a11y` (dev server
   must already be running). Report the score and flag anything it
   surfaces that axe didn't (mainly tap-target size and viewport meta
   checks — most of its rule set overlaps axe-core, so don't re-report
   duplicates).
5. **Always remind the developer of what neither tool covers** — pull from
   `.claude/skills/4-tdd/a11y-checklist.md` and
   ...
   - Custom drag interactions needing a keyboard equivalent
...
- [pass/fail] <other interactive-open states this project has>

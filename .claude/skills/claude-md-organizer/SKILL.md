...
for many projects it's the primary human-facing entry point (a recruiter
for a portfolio, a contributor for an OSS repo, a teammate for internal
tooling). Content that's genuinely reader-facing (an architecture
diagram, for instance) belongs directly in the README where it will
actually be seen, not filed away in `docs/reference/` just because the
same "keep it out of CLAUDE.md" instinct also feels applicable there.
...
- `4-tdd/` — implements one issue via TDD; must contain `SKILL.md`,
  `fe-standards.md`, `fe-testing-patterns.md`, `a11y-checklist.md`,
  `pre-commit.md`, and — only if
